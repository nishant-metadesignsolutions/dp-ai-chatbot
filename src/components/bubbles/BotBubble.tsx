...
export const BotBubble = (props: Props) => {
  ...
  createEffect(() => {
    if (botMessageEl && props.isLastMessage) {
      const targets = botMessageEl.querySelectorAll('p, li');
      targets.forEach((el, i) => {
        el.classList.remove('animate'); // force reflow
          ...
          {props.message.message && (
            <>
              {!props.isLastMessage && (
                <div
                  // ref={botMessageEl}
                  class="font-dubai px-5 py-3 ml-2 max-w-full chatbot-host-bubble prose mb-5"
              ...
              {props.isLastMessage && (
                <div
                  // ref={botMessageEl}
                  class="font-dubai typing-text px-5 py-3 ml-2 max-w-full chatbot-host-bubble prose mb-5"
                  // data-testid="host-bubble"
                  style={{
                    'background-color': props.backgroundColor ?? defaultBackgroundColor,
                    color: props.textColor ?? defaultTextColor,
